...
export function TrustBar() {
  return (
    <div className="border-b border-slate-200 bg-white px-4 py-3.5 sm:px-6 lg:px-8 dark:border-slate-800 dark:bg-slate-950">
      <div className="mx-auto max-w-7xl">
        <ul className="flex flex-wrap items-center justify-center gap-x-6 gap-y-2">
          {signals.map(({ icon: Icon, text }, i) => (
            <li key={text} className="flex items-center gap-1.5">
              {i > 0 && (
                <span className="mr-4 hidden h-3.5 w-px bg-slate-200 sm:inline-block dark:bg-slate-800" aria-hidden />
              )}
