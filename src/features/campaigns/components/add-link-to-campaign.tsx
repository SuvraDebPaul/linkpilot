...
export function AddLinkToCampaign({ campaignId, userId }: { campaignId: string; userId: string }) {
  ...
  async function handleAdd() {
    ...
    setIsPending(false);
    if (result.success) { toast.success("Link added to campaign."); setOpen(false); setSelected(""); }
        ...
        <div className="space-y-4 pt-2">
          <div className="space-y-1.5">
            ...
            <Select value={selected} onValueChange={setSelected}>
              ...
              <SelectContent>
                {links.length === 0 ? (
                  <SelectItem value="__none" disabled>No available links</SelectItem>
                ) : (
                  links.map((l) => (
            ...
            <p className="text-xs text-muted-foreground">Only links not already in a campaign are shown.</p>
          </div>
